...
DEFAULT_OUTPUT_PATH = "output/subito_jobs.json"
DEFAULT_LIMIT = 5
DEFAULT_TOP = 50
REQUEST_TIMEOUT = 15
USER_AGENT = (
...
DIRECT_QUERIES = [
    "lavoro part time Napoli",
    "cameriere part time Napoli",
    "pulizie part time Napoli",
    "magazziniere part time Napoli",
    "lavoro Pozzuoli",
    "lavoro Bacoli",
...
def build_subito_search_url(query):
    return f"{SUBITO_BASE_URL}/annunci-campania/vendita/offerte-lavoro/?q={quote_plus(query)}"
...
def fetch_direct_search(url):
    try:
        ...
        print(f"Subito direct search failed: {url} | {exc}")
        return None

    if is_blocked_response(response):
        print(f"Subito direct search blocked or unavailable: {url} | status {response.status_code}")
        return None
    if response.status_code >= 400:
        print(f"Subito direct search failed: {url} | status {response.status_code}")
        return None
...
def is_subito_job_detail_url(url):
    normalized = normalize_url(url)
    return "subito.it" in normalized and "/offerte-lavoro/" in normalized and normalized.endswith(".htm")


def parse_subito_html(page_html, query):
    jobs = []
    seen_urls = set()
    for match in re.finditer(r'<a[^>]+href="([^"]+)"[^>]*>(.*?)</a>', page_html, re.DOTALL | re.IGNORECASE):
        href = html.unescape(match.group(1))
        url = normalize_url(urljoin(SUBITO_BASE_URL, href))
        if not is_subito_job_detail_url(url) or url in seen_urls:
            continue

        title = anchor_text(match.group(2))
        if not title:
        ...
        seen_urls.add(url)
        jobs.append(normalize_subito_result({
            "title": title,
            "company": "",
            "location": "",
            "url": url,
...
def extract_title_from_url(url):
    ...
    title = re.sub(r"-\d+\.htm$", "", path)
    title = title.replace(".htm", "").replace("-", " ")
    return clean_text(title)


def normalize_subito_result(result, query, found_at=None):
...
def collect_direct_jobs(limit=DEFAULT_LIMIT, pause_seconds=3):
    ...
    for query in DIRECT_QUERIES:
        search_url = build_subito_search_url(query)
        print(f"Subito direct search: {search_url}")
        page_html = fetch_direct_search(search_url)
        if page_html:
            for job in parse_subito_html(page_html, query):
